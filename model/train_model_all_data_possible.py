...
def extract_from_tsv(tsv_file_path: str) -> List[str]:
    first_column = []
    os_path_object = os.path.join(OS_PATH_DATASET_TEXTS_ONLY_TEST_SPECIAL, tsv_file_path)
...
# add test
hw_texts = hw_texts + load_dataset(FILENAME_HW_DATASET_TEXTS_TEST)
hw_labels = [0]*len(hw_texts)

mg_texts_mixed = load_dataset(FILENAME_MG_DATASET_TEXTS_TRAIN_EVAL_MIXED)
# add llama
mg_texts_mixed = mg_texts_mixed + load_dataset(FILENAME_MG_DATASET_TEXTS_TRAIN_EVAL_LLAMA)
# add test mixed
mg_texts_mixed = mg_texts_mixed + load_dataset(FILENAME_MG_DATASET_TEXTS_TEST_MIXED)
# add test llama
mg_texts_mixed = mg_texts_mixed + load_dataset(FILENAME_MG_DATASET_TEXTS_TEST_MIXED)
# add test gpt
mg_texts_mixed = mg_texts_mixed + extract_from_tsv(FILENAME_MG_DATASET_TEXTS_TEST_SPECIAL_GPT)
# add test mistral
mg_texts_mixed = mg_texts_mixed + extract_from_tsv(FILENAME_MG_DATASET_TEXTS_TEST_SPECIAL_MISTRAL)
