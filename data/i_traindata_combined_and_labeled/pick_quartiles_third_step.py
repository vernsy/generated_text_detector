...
### load all dicts
# load jsons
def load_json_dict_indices_pool(FILENAME: str) -> Dict[str, Dict[str, int]]:
...
def get_indices_per_quartiles(
    ...
    dict_pool_quartiles_indices_lists_hw = load_json_dict_indices_pool(
                                                FILENAME_JSON_INDICES_DICT_HW_TRAIN_EVAL_POOL)
    dict_pool_quartiles_indices_lists_mg = load_json_dict_indices_pool(
                                                FILENAME_JSON_INDICES_DICT_MG_TRAIN_EVAL_POOL_LLAMA)
    
    if DESIRED_NUM_OF_INDICES_PER_CLASS_HW == None:
        dict_shortened_quartiles_indices_lists_hw = dict_pool_quartiles_indices_lists_hw
    else:
        dict_shortened_quartiles_indices_lists_hw = kick_out_random_until_required_length(
            dict_pool_quartiles_indices_lists_hw,
            DESIRED_NUM_OF_INDICES_PER_CLASS_HW
        ) 
    if save_indices_dictionary:
        save_dicts_updated_indices_per_genre_and_quartile(FILENAME_JSON_INDICES_DICT_HW_TRAIN_EVAL,
                                                        dict_shortened_quartiles_indices_lists_hw
                                                        )
    if DESIRED_NUM_OF_INDICES_PER_CLASS_MG == None:
        dict_shortened_quartiles_indices_lists_mg = dict_pool_quartiles_indices_lists_mg
    else:
        dict_shortened_quartiles_indices_lists_mg = kick_out_random_until_required_length(
            dict_pool_quartiles_indices_lists_mg,
            DESIRED_NUM_OF_INDICES_PER_CLASS_MG
        )
    if save_indices_dictionary:
        save_dicts_updated_indices_per_genre_and_quartile(FILENAME_JSON_INDICES_DICT_MG_TRAIN_EVAL_LLAMA,
...
if __name__ == "__main__":
    get_indices_per_quartiles(22500,22501,True)
